...
class Constants:
  ...
  RemB = 1.e+6/me/Bo      # i.e. 1 MeV / Bo / me       
...
class EDGE(Constants):
  ...
  def __init__(self, scalefile, cfg_file):
    ...
    self.cc      = ROOT.TCanvas('cc','BEMS for BEPC-II', 800, 600, 800, 600)
  ...
  def Go(self, UTB, UTE, hps, filechain):
    ...
    if self.VEPP2K:
      ...
      if gain:
        ...
        if Wmax:
          Wmax = self.fitEdgeComple(Wmax, self.Ranger)
...
# +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
  def fitEdgeSimple(self,W,LBK):
    ...
    if not R.Status():
      ...
      print ' ║ Range from %6.1f to %6.1f keV │ E_beam = %7.2f MeV │   W_max = %9.3f keV     ║' % (E1, E2, self.Eo, W)
      print ' ╟─────────────────────────────────┴────────┬─────────────┴─────────────────────────────╢'
...
# +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
  def fitEdgeComple(self,W,LBK):
    ...
    if not R.Status():
      ...
      BE, dBE  = E['p'][0]-1e-3*self.SC*deriv,  (E['e'][0]**2 + (1e-3*deriv*self.dSC)**2)**0.5
      BS, dBS  = E['p'][9]*deriv,  E['e'][9]*deriv # beam energy spread [MeV]
      Ro       = 1.e+6*E['p'][0]/Constants.c/E['p'][1] * 100 # Beam radius [cm] 
      dRo      = Ro * ((E['e'][0]/E['p'][0])**2 + (E['e'][1]/E['p'][1])**2)**0.5
      print ' ╔ Convolution Fit: ═══════════════╤══════════════════════╤═════════════════════════════╗' 
      print ' ║ Range from %6.1f to %6.1f keV │ σR = %4.2f ± %4.2f keV │ σL = %5.2f ± %5.2f keV      ║' % (E1, E2, self.RR, self.dRR, self.RL, self.dRL)
      print ' ╟─────────────────────────────────┴────────┬─────────────┴─────────────────────────────╢'
      print ' ║ Beam energy: %8.3f ± %5.3f [MeV]      │ Bending field: %6.4f ± %6.4f [T]        ║' % (BE, dBE, E['p'][1], E['e'][1])
      print ' ║ σ from beam: %8.3f ± %5.3f [keV]      │ Beam spread:   %6.0f ± %6.0f [MeV]      ║' % (E['p'][9], E['e'][9], BS, dBS)
      print ' ║ Edge tilt pol1: %5.3f ± %5.3f [1/eV]     │ Edge tilt pol2: %5.2f ± %5.2f  [1/eV^2]   ║' % tilt
      print ' ╟──────────────────────────────────────────┼───────────────────────────────────────────╢'
      print ' ║         χ2/NDF = %5.1f/%3d               │         Probability: %8.6f             ║' % (R.Chi2(), R.Ndf(), R.Prob())
      print ' ╚══════════════════════════════════════════╧═══════════════════════════════════════════╝\n'
      ...
      self.Legend.AddEntry(self.comple, '#sigma_{E} = %6.0f #pm %4.0f [MeV]' % (BS, dBS), 'l')
      self.Legend.AddEntry(self.comple, 'R_{beam} = %6.2f #pm %5.2f [cm]'    % (Ro, dRo), 'l')
